...
export const classesServices = {
  getClassDivisions: async (token: string): Promise<ApiResponse<{ divisions: ClassDivision[]; total_divisions: number; total_students: number }>> => {
    return apiClient.get('/api/students/divisions/summary', token);
  },

  // Get students by class division
  getStudentsByClass: async (classDivisionId: string, token: string): Promise<ApiResponse<{ students: Student[]; count: number }>> => {
    return apiClient.get(`/api/students/class/${classDivisionId}`, token);
  },

  // Get a specific student details
  getStudentDetails: async (studentId: string, token: string): Promise<ApiResponse<StudentDetailsResponse>> => {
