...
export const leaveRequestServices = {
  ...
  create: async (data: CreateLeaveRequest, token: string): Promise<LeaveRequestResponse> => {
  ...
  list: async (params: ListLeaveRequestsParams = {}, token: string): Promise<LeaveRequestsResponse> => {
  ...
  getById: async (id: string, token: string): Promise<LeaveRequestResponse> => {
  ...
  updateStatus: async (id: string, data: UpdateLeaveRequestStatus, token: string): Promise<LeaveRequestResponse> => {
  ...
  approve: async (id: string, token: string): Promise<LeaveRequestResponse> => {
    return leaveRequestServices.updateStatus(id, { status: 'approved' }, token);
  },

  // Reject a leave request
  reject: async (id: string, rejection_reason: string, token: string): Promise<LeaveRequestResponse> => {
  ...
  getByStudent: async (studentId: string, params: Omit<ListLeaveRequestsParams, 'student_id'> = {}, token: string): Promise<LeaveRequestsResponse> => {
    return leaveRequestServices.list({ ...params, student_id: studentId }, token);
  },

  // Get leave requests for a specific class division
  getByClass: async (classDivisionId: string, params: Omit<ListLeaveRequestsParams, 'class_division_id'> = {}, token: string): Promise<LeaveRequestsResponse> => {
